<template>
    ...
    <header class="db-header">
        <router-link class="w-32 flex-shrink-0" :to="{ name: 'admin.dashboard' }">
            <img class="h-10 max-h-10 object-contain rounded-xl p-1 bg-white border border-gray-200/50 dark:border-white/20 shadow-xs transition-all" :src="setting.theme_logo" alt="logo">
        </router-link>
...
export default {
    ...
    beforeMount() {
        ...
        if ((env.DEMO === "true" || env.DEMO === true || env.DEMO === "1" || env.DEMO === 1) && (this.$router.currentRoute.name !== "frontend.socialLogin")) {
            this.$store.dispatch("authcheck").then(res => {
                if (res.data.status === false) {
                    alertService.error(this.$t('message.demo_login_expired'));
