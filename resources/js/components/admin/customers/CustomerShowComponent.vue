<template>
    ...
    <div class="col-12" v-if="Object.entries(customer).length > 0">
        ...
        <div id="profile" class="profile-tabDiv active">
            ...
            <!-- Monnify Account Section for Admin -->
            <div class="db-card mt-6">
                <div class="db-card-header flex items-center justify-between">
                    <h3 class="db-card-title flex items-center gap-2">
                        <i class="lab lab-line-card text-primary text-xl"></i> Monnify Virtual Account
                    </h3>
                    <div v-if="customer.monnify_account_number">
                        <button type="button" @click="deleteMonnifyAccount" class="db-btn-outline !text-danger !border-danger hover:!bg-danger hover:!text-white h-9 py-1 px-3 text-xs gap-1.5 rounded-lg transition-all">
                            <i class="lab lab-line-trash text-sm"></i> Delete Account Number
                        </button>
                    </div>
                    <div v-else>
                        <button type="button" @click="generateMonnifyAccount" class="db-btn bg-primary text-white h-9 py-1 px-3 text-xs gap-1.5 rounded-lg transition-all">
                            <i class="lab lab-line-circle-plus text-sm"></i> Generate Account Number
                        </button>
                    </div>
                </div>
                <div class="db-card-body">
                    <div v-if="customer.monnify_account_number" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <span class="block text-xs text-gray-500 font-medium mb-1">Bank Name</span>
                            <span class="block text-sm font-bold text-gray-800">{{ customer.monnify_bank_name || 'Wema Bank / Monnify' }}</span>
                        </div>
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <span class="block text-xs text-gray-500 font-medium mb-1">Account Number</span>
                            <span class="block text-base font-mono font-bold text-primary">{{ customer.monnify_account_number }}</span>
                        </div>
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <span class="block text-xs text-gray-500 font-medium mb-1">Account Name</span>
                            <span class="block text-sm font-bold text-gray-800">{{ customer.monnify_account_name }}</span>
                        </div>
                    </div>
                    <div v-else class="text-gray-500 text-sm italic py-2">
                        No Monnify virtual account has been assigned to this customer yet. Click 'Generate Account Number' to create one manually or it will auto-generate when the customer logs in / visits their wallet.
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="db-card">
                ...
                <div class="db-card-body">
                    <div class="row">
                        ...
                        <div class="db-table-body w-full" v-else>
                            <div class="db-table-body-tr">
                                <div class="db-table-body-td text-center">
                                    <div class="p-4">
                ...
                <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-6" v-if="myOrders.length > 0">
...
export default {
    ...
    methods: {
        ...
        deleteMonnifyAccount: function () {
            if (confirm("Are you sure you want to delete this customer's Monnify virtual account number?")) {
                this.loading.isActive = true;
                axios.post(`admin/customer/delete-monnify-account/${this.$route.params.id}`).then((res) => {
                    this.loading.isActive = false;
                    alertService.success("Monnify account number deleted successfully!");
                    this.$store.dispatch("customer/show", this.$route.params.id);
                }).catch((err) => {
                    this.loading.isActive = false;
                    alertService.error(err.response?.data?.message || "Failed to delete account number");
                });
            }
        },
        generateMonnifyAccount: function () {
            this.loading.isActive = true;
            axios.post(`admin/customer/generate-monnify-account/${this.$route.params.id}`).then((res) => {
                this.loading.isActive = false;
                alertService.success("Monnify virtual account number generated successfully!");
                this.$store.dispatch("customer/show", this.$route.params.id);
            }).catch((err) => {
                this.loading.isActive = false;
                alertService.error(err.response?.data?.message || "Failed to generate account number");
            });
        },
    },
};
